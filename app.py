...
def load_stocks():
    try:
        ...
        # 🌟 修復核心：強制轉文字，並清掉 Google 表單可能傳回來的隱形單引號
        df["代號"] = df["代號"].astype(str).str.lstrip("'")
        return df
...
def save_all_data(df_stocks, df_funds):
    try:
        # 🌟 修復核心：存檔前，強制在所有代號前面加上單引號(')，逼 Google 當純文字存！
...
stock_col_config = {
    ...
    "代號": st.column_config.TextColumn("代號 (台股ETF請補齊前面的0)", required=True),
